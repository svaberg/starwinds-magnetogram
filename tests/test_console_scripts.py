...
@pytest.mark.parametrize("options",
                         ("-p map",
                          ...
                          "-p azimuth -v",
                          "-p radial -v",))
def test_sw_plot_magnetogram(script_runner, options, request):
    file = context.default_artifact_directory + '/test_field_zdipy.dat'

    pn = context.PlotNamer(__file__, request.node.name)

    ret = script_runner.run('sw-plot-magnetogram', file, pn.get().replace(' ', ''), *options.split(" "))
...
def test_sw_convert_magnetogram(script_runner, request, inverse, format_only, degree_max):
    file = context.default_artifact_directory + '/test_field_zdipy.dat'
    pn = context.PlotNamer(__file__, request.node.name)
