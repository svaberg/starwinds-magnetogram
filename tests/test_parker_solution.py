...
def test_1(request):
    ...
    parker_solution = ParkerSolution()
    ...
    with context.PlotNamer(__file__, request.node.name) as (pn, plt):
        ...
        for T_id, T in enumerate(Ts):
            ...
            plt.text(r[-1], u[-1], '%1.1f MK' % (1e-6 * T), color=c,
...
def test_2(request):
    ...
    with context.PlotNamer(__file__, request.node.name) as (pn, plt):
        ...
        for _id, coronal_base_temperature in enumerate(coronal_base_temperatures):
            ...
            ps = ParkerSolution(temperature=coronal_base_temperature)

            r = np.geomspace(ps.stellar_radius, 215 * ps.stellar_radius)  # 20*r_S)
...
def test_total_mass_flux(request):
    ...
    with context.PlotNamer(__file__, request.node.name) as (pn, plt):
        ...
        for _id, coronal_base_temperature in enumerate(coronal_base_temperatures):
            ...
            parker_solution = ParkerSolution(temperature=coronal_base_temperature)

            r = np.geomspace(parker_solution.stellar_radius, 215 * parker_solution.stellar_radius)  # 20*r_S)
            c = plt.rcParams['axes.prop_cycle'].by_key()['color'][_id]

            u = parker_solution.speed(r)
            rho = parker_solution.density(r)
            r_sonic = parker_solution.radius_sonic
            u_sonic = parker_solution.speed_sonic
            rho_sonic = parker_solution.density_sonic

            total_mass_flux = r**2 * u * rho
            total_mass_flux_sonic = r_sonic**2 * u_sonic * rho_sonic

            ax1.plot(r/parker_solution.stellar_radius, total_mass_flux, color=c)
            ax1.plot(r_sonic/parker_solution.stellar_radius, total_mass_flux_sonic, 'o', color=c)

            ax2.plot(r / parker_solution.stellar_radius,
                     total_mass_flux / parker_solution.stellar_mass * 3600 * 24 * 365.25,
                     color=c)
...
def test_total_mass_flux2(request):
    ...
    with context.PlotNamer(__file__, request.node.name) as (pn, plt):
        ...
        for coronal_base_number_density in coronal_base_number_densities:
            ...
            parker_solution = ParkerSolution(coronal_base_temperatures,
                                             base_density=coronal_base_number_density * scipy.constants.proton_mass)

            radius_sonic = parker_solution.radius_sonic
            speed_sonic = parker_solution.speed_sonic
            density_sonic = parker_solution.density_sonic
        ...
        for coronal_base_number_density in coronal_base_number_densities:
            ...
            radius_sonic = parker_solution.speed_sonic
            speed_sonic = parker_solution.radius_sonic
            density_sonic = parker_solution.density_sonic

            total_mass_flux = radius_sonic**2 * speed_sonic * density_sonic

            ax.plot(coronal_base_temperatures, total_mass_flux / parker_solution.stellar_mass * 3600 * 24 * 365.25)
